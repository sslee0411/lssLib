...
/// <para>RS-485 환경에서는 반드시 <c>SequenceMode=1 (Sequential)</c> 로 설정하세요 (버스 충돌 방지).</para>
///
/// <b>생성 방법:</b>
/// <code>
/// // FromConfig 팩토리 권장 (DataBits/Parity/Timeout 자동 적용, LogSource 자동 주입)
...
public sealed class SerialTransport : NetTransportBase
{
    ...
    /// <summary>SerialDeviceConfig 에서 생성합니다. DeviceName → LogSource 자동 주입.</summary>
    ...
    protected override Task ConnectCoreAsync(CancellationToken ct)
    {
        ...
        // Passive 수신: DataReceived 이벤트 → RaiseDataReceived → NetChannelBase
