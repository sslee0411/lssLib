...
public sealed class TcpTransport : NetTransportBase
{
    ...
    /// true = 백그라운드 수신 루프 활성화 (PassiveNetChannel 필수).<br/>
    ...
    /// <summary>
    /// TcpDeviceConfig 에서 생성합니다. DeviceName → LogSource 자동 주입.
    /// </summary>
    ...
    protected override async Task ConnectCoreAsync(CancellationToken ct)
    {
        ...
        await _client.ConnectAsync(_strHost, _port, timeoutCts.Token)
    ...
    /// <b>연결 끊김 감지:</b>
    /// <list type="bullet">
    ///   <item><description>read == 0 → 서버 정상 종료</description></item>
    ///   <item><description>IOException → 서버 강제 종료 / 네트워크 단절</description></item>
    ///   <item><description>→ State = Error → OnStateChanged(Error) → HandleErrorAsync → 재접속</description></item>
    /// </list>
    /// </summary>
    private async Task PassiveReceiveLoopAsync(CancellationToken ct)
    {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                if (read == 0)
                    throw new IOException(
                        $"[{LogSource}] 서버가 연결을 종료했습니다. (read=0)");
        ...
        {
            // 정상 종료 (DisconnectAsync / DisposeAsync 호출 시)
        }
        catch (Exception) when (!ct.IsCancellationRequested)
        {
            // 예기치 못한 연결 끊김 → State = Error → 재접속 트리거
            // DisconnectAsync 는 NetConnectionManager 가 처리하므로 State 변경만
