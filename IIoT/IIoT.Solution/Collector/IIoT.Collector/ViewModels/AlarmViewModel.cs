...
public partial class AlarmViewModel : ObservableObject, IDisposable
{
    ...
    public AlarmViewModel(AlarmStateManager alarmManager, AlarmShelvingService shelving)
    {
    ...
    /// <summary>선택한 알람의 Tag 를 1시간 동안 알림 억제 (C-EX-05 신규)</summary>
    [RelayCommand]
    private void ShelveAlarm(AlarmRowViewModel? row)
    {
        if (row is null) return;
        _shelving.Shelve(row.TagId, minutes: 60);
    }
