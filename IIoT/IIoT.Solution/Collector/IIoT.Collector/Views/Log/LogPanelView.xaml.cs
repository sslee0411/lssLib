// ══════════════════════════════════════════════════════════
//  IIoT.Collector · Views/Log/LogPanelView.xaml.cs
//  역할: 하단 로그 패널 코드비하인드
//        LogManager.Instance.LogAdded 이벤트 구독 → ListView 표시
//  Col-Base-0: Studio-P04 LogPanelView.xaml.cs 패턴 그대로 복사
//              (네임스페이스만 IIoT.Collector.Views.Log 로 변경)
//
//  ★ 확인된 오류 방지 패턴 (Studio-P04 fix 기준):
//    CS0246  ICollectionView → using System.Windows.Data 필수
//    CS1503  data.Time 은 이미 string → ToString() 호출 금지
//    CS1061  data.Message 없음 → data.Contents 사용
//    NullRef OnFilterChanged → LevelFilter/SourceFilter null 가드 필수
//    로그 미표시 → LoadPlugins() 를 win.Loaded 안으로 이동 (App.xaml.cs 에서 처리)
//
//  생성: 2026-06-29
// ══════════════════════════════════════════════════════════
...
using System.Windows.Data;          // ★ CS0246: ICollectionView 참조 필수

namespace IIoT.Collector.Views.Log;

/// <summary>로그 한 줄을 ListView 에 바인딩하기 위한 뷰 모델</summary>
...
public partial class LogPanelView : UserControl
{
    ...
    public LogPanelView()
    {
        ...
        //   ComboBox IsSelected="True" 파싱 시 OnFilterChanged 발화 → null 가드 필요
    ...
    private void _OnLogAdded(LogData data)
    {
        ...
        Dispatcher.InvokeAsync(() =>
        {
            var row = new LogRow
            {
                // ★ CS1503 방지: data.Time 은 이미 "HH:mm:ss.fff" string
                TimeText = data.Time,
                LevelText = data.LevelText,
                Source = data.Source ?? string.Empty,
                // ★ CS1061 방지: Message 프로퍼티 없음 → Contents 사용
    ...
    {
        // ★ NullRef 방지: XAML 파싱 중 이벤트 발화 시 null 가드 필수
