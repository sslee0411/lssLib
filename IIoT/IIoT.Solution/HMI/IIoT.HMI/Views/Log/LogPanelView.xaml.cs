// ══════════════════════════════════════════════════════════
//  IIoT.HMI · Views/Log/LogPanelView.xaml.cs
//  역할: [로그] 탭 코드비하인드
//        LogManager.Instance.LogAdded 이벤트 구독 → ListView 표시
//        Studio-P04 / Collector Col-Base-0 / Monitor(2026-07-08) LogPanelView.xaml.cs
//        패턴 그대로 포팅(네임스페이스만 IIoT.HMI.Views.Log 로 변경)
//
//  ★ 이식 시 이미 확인된 오류 방지 패턴 (Studio-P04 fix 기준):
...
using System.Windows.Data;          // ★ CS0246 방지: ICollectionView 참조 필수

namespace IIoT.HMI.Views.Log;
...
public partial class LogPanelView : UserControl
{
    ...
    private void _OnLogAdded(LogData data)
    {
        ...
        Dispatcher.InvokeAsync(() =>
        {
            var row = new LogRow
            {
                // ★ CS1503 방지: data.Time 은 이미 "HH:mm:ss.fff" 형식 string
                TimeText = data.Time,
