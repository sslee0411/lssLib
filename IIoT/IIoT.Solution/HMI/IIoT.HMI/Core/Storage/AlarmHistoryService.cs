// ══════════════════════════════════════════════════════════
//  IIoT.HMI · Core/Storage/AlarmHistoryService.cs
//  역할: AlarmAggregator.AlarmRecorded 이벤트(생성+상태전이 모두)를 구독하여
//        SQLite alarm_history 테이블에 영구 저장한다. Collector의
//        AuditLogService.cs / Monitor의 AlarmHistoryService.cs 와 동일한
//        패턴(자체 SqliteDbContext 관리). 재시작해도 알람 이력이 사라지지
//        않도록 하는 것이 목적(실무강화).
//        (IIoT.Monitor Core/Storage/AlarmHistoryService.cs — MN-EX-02 이식,
//         필드/동작 동일)
//        ★ Monitor 원본과 동일하게 "저장 전용" 범위다 — 이력을 화면에서
//        조회/검색하는 UI는 없음(alarm_history.db 를 외부 SQLite 툴로 열어
//        확인하는 방식). 조회 UI가 필요하면 별도 Step으로 추가 검토.
//  HM-16: 신규
//  생성: 2026-07-19
// ══════════════════════════════════════════════════════════
...
namespace IIoT.HMI.Core.Storage;
...
public sealed class AlarmHistoryService : IAsyncDisposable
{
    ...
    /// <summary>
    /// SQLite DB 를 열고 테이블을 초기화한다. 앱 시작 시(App.xaml.cs OnStartup) 1회 호출.
    /// </summary>
    public async Task InitializeAsync()
    {
        var dbPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Data", "hmi.db");
