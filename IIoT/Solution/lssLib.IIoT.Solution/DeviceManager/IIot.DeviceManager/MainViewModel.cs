        ...
        try
        {
            await Task.Run(() =>
            {
                ...
                System.Windows.Application.Current.Dispatcher.Invoke(() =>
                {
                    ...
                    if (!integrityOk)
                        SaveStatus = $"⚠ 무결성 경고 — device.json 외부 변경 감지 " +
                                     $"({nodes.Count}개 루트 로드됨)";
                    else
                        SaveStatus = $"로드 완료 — {DeviceTree.TotalNodeCount}개 노드";
                });
            });
